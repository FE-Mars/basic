    ...
    <div class="page-result">
        <page-header title="">
            <el-form slot="content" :inline="true" :model="filters">
                <el-form-item label="文件名称(SXI)">
                    <el-input
                        v-model="filters.fileNameSxi"
                        placeholder="文件名称(SXI)"
                        clearable
                    />
                </el-form-item>
        ...
        <page-main>
            <div class="list-wrap">
                ...
                <el-table :data="list" @selection-change="onSelectionChange">
                    ...
                    <el-table-column prop="fileNameEsac" label="文件名称（ESAC）" />
                    <el-table-column prop="fileNameSxi" label="文件名称（SXI处理子系统）" />
                    <el-table-column prop="compareTime" label="对比时间" />
                    <el-table-column label="操作" width="250px">
                        <template slot-scope="scope">
                            <el-link :underline="false" type="primary" :href="`${VUE_APP_API_ROOT}/dataCompare/download/${scope.row.id}`" :download="scope.row.fileNameSxi + '.cdf'" target="_blank" rel="下载文件" @click.stop>
